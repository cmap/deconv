...
palette(c("dodgerblue","brown","orange"))
...
```{r kd_barplot, echo=F, fig.cap=cap, fig.width=8}
cap <- "\\label{kd_barplot}**KD success frequency**" 
...
with(speed_acc[!is.na(kd_success_freq)][
  ...
  bp <- barplot(kd_success_freq, names.arg=handle, las=3,
...
# TODO: better caption
cap <- "\\label{tsne_figure}**t-SNE projection of holdout data.** Each point represents the 2D projection of a sample generated by UNI ground truth (GT) or by applying a deconvolution algorithm to DUO data. DECONV data colored by perturbagen type (A) and algorithm type (B). DECONV (C) and DE (D) data colored by algorithm type and stratified by each individual implementation."
