...
cap <- "Accuracy vs speed, stratified by plate. Dashed lines indicate benchmark."
par(mfrow=c(2, 2))
for (k in 1:2) {
j <- plate==unique(plate)[k]
# as percent over benchm
bench <- auc[j & handle=="benchmark"]
bench.sec <- sec[j & handle=="benchmark"]
ylim <- range(c(1, (auc/bench)[handle!="benchmark"]))
xlim <- log(range(c(1, (sec/bench.sec)[handle!="benchmark"])))
# plot panel 
...
text(x=log(sec/bench.sec)[j & handle!="benchmark"]
  ...
  , labels=impute(method_short,"other")[j & handle!="benchmark"]
  , pos=1, xpd=T)
...
ylim <- range(c(1, (cor/bench)[j & handle!="benchmark"]))
