...
```{r accuracy-fig, fig.cap = cap, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
...
panel <- function(x, ...) {
  ...
  txt <- gsub(".*:", "", colnames(x))
  b <- boxplot(x, outline = F, names = txt, las = 2, notch = T, lty=1, ...)
  grid(nx=NA, ny=NULL, col = gray(.25))
  boxplot(x, outline = F, names = txt, las = 2, notch = T, lty=1, add=T, col = rgb(1, .5, 0, .5), ...)
}
...
panel.bottom (ds_cor@mat[, 1:10], main = "A. shRNA", xlab = expression(rho))
panel.bottom (ds_cor@mat[, 11:20], main = "B. compounds", xlab = expression(rho))
par(mar = c(6, 3, 2, 1) + 0.1)
panel (ds_cor@mat[, 1:10]  - ds_cor@mat[, 4], main = "C. shRNA")
panel (ds_cor@mat[, 11:20]  - ds_cor@mat[, 14], main = "D. compounds")
```
