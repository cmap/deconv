...
gt.1 <- gt_ds@mat[1:5, ] # one sample
bench.1 <- bench_ds@mat[1:5, ]
uni <- apply(gt.1, 1, function(x) c(Est=mean(x),SE=sd(x)))
duo.bench <- apply(bench.1, 1, function(x) c(Est=mean(x),SE=sd(x)))
knitr::kable(rbind(uni, duo.bench), digits=0)
