...
tmp <- rbind(ens_corr[!grepl("benchmark", algos)], 
             speed_acc[plate=="DPK.CP003_PC3_24H_X1_B42", .(algos, cor, sec, nalgo)],
             fill=T, use.names=T)
```

```{r ensemble_plot, echo=F}
ggplot(tmp, aes(x=sec, y=cor)) +
  geom_point(aes(size=nalgo), alpha=0.6) +
  geom_point(data=tmp[nalgo==1],
             aes(x=sec, y=cor, color=algos)) +
  scale_size_continuous(breaks = 1:9) +
  xlab("cumulative runtime (sec)") +
  ylab("correlation score")
```

```{r ensemble_plot2, echo=F}
ggplot(ens_corr, aes(x=nalgo, y=cor, group=nalgo)) +
  geom_boxplot() +
