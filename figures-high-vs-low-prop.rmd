...
```{r}
panel <- function(x, type="experiments", prob = c(0.025,0.5, 0.975), ...) {
  margin <- switch(type, genes = 1, experiments = 2)
  y <- apply(x, margin, quantile, p = prob)
  matplot(t(y), type="l", ...)
#   txt <- paste("mean =", round(mean(y),2))
#   par(usr=c(0,1,0,1))
#   text(.5,.9,txt)
}
par(mfrow=c(4, 3))
for (j in names(out))
  panel(out[[j]]$var, ylim=c(0, 0.35), main = j)
```
...
par(mfrow=c(4, 3))
for (j in names(out)){
  panel(log(out[[j]]$var / out[["ground-truth"]]$var)
      , type = "genes"
      , main = j
      , ylim = c(-1, 1) * 5
#       , ylim = 100*c(0, 2.5)
      , prob = c(0.5, 0.95, 0.05)
      , lty = c(1, 2, 2), lwd=c(1, 1, 1), col = 2)
  abline(v=n.high, col=1)
  abline(h=0, lty=2)
}
```
...
```{r}
par(mfrow=c(3,2))
boxplot(t(out$gardn999$var[,low]),pch=16,ylim=c(0,5))
title()
boxplot(t(out$benchmark$var[,low]),pch=16,ylim=c(0,5))
boxplot(t(out$gardn999$var[,high]),pch=16,ylim=c(0,5))
boxplot(t(out$gardn999$var[,high]),pch=16,ylim=c(0,5))
 barplot(1:12) # grouop by low-vs-high by algo
 barplot(1:12)
```
