...
CMap’s current deconvolution algorithm (called dpeak) is based on the k-means clustering algorithm, which is a flexible and powerful non-parametric tool widely used in biology, as well as in many other fields. This approach automatically partitions a set of gene-expression measurements into $k$ clusters by minimizing the within-cluster sum of squares, and then assigns the two largest clusters to each gene type within each matched pair of genes. 
