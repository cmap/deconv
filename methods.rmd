...
Towards developing an open innovation competition, we first transformed the deconvolution problem into a supervised classification task. Using the L1000 assay, we profiled six 384-well plates, each containing different sets of compound and shRNA treatments (see Supporting Info, \ref{data-gen}). We did it twice by varying the detection mode used for acquiring the data between two methods. The first method, called UNI, associates each gene to one single analyte color, which leads to higher costs, but it does not require deconvolution.
The second method, called DUO, associates sets of two genes per analyte color, which reduces costs but requires deconvolution to transform the composite gene-expression signal into two separate gene-specific expression values.
We then used the data obtained from the UNI method (one gene per analyte type) as the "ground truth" for the competition; and the data obtained from the DUO method (two genes per analyte type) as the input to predict the equivalent UNI data. The datasets are now publicly available [LINK to REPO]. 
