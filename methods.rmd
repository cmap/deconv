...
The first step towards developing an open innovation competition was to transform the deconvolution problem into a supervised classification task. We generated a novel L1000 dataset of gene-expression values (for 976 genes and xxx compounds and xxx shRNA) based on a UNI detection method (one bead type per gene). This detection method does not require the deconvolution step, because it has one bead type per gene, and produced data that was suitable as the ground-truth data for the contest. Then, on the same samples, we generated additional L1000 data with the current DUO detection method (two genes per bead type). 

Contestants received about 2/3 of DUO and UNI data as input to train and test their approaches, the remaining 1/3 was used for final testing (to avoid overfitting). All datasets are now publicly available [LINK to REPO]. 

We developed a scoring function that combines standard measures of accuracy (spearman rank correlation between algorithmic predictions and the ground-truth data; AUC between extreme-modulations predicted based on DUO data and observed extreme modulations in UNI data), as well as computational speed to evaluate the submissions received and award the prizes to the best competitors. 

Finally, the open innovation competition was run on tge online platform Topcoder (Wipro, India), for a total of 10 days, offering $10,000 in cash prizes as incentive to be divided amont the top 9 submissions (prize split are: 5, 3, 2, 1, 500, ...). 
