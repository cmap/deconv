...
The first step towards developing an open innovation competition was to transform the deconvolution problem into a supervised classification task. 

Using the L1000 assay, we profiled six 384-well plates, each containing different sets of compound and shRNA treatments (Supporting Info, xxxx). These plates were profiled using different detection methods. 
The first detection method, called UNI, measured each gene individually by assigning all genes to mutually exclusive bead colors. Deconvolution was unnecessary to extract the expression values under this method.
The other method, called DUO, measured a composite expression signal of non-overlapping pairs of genes using the same analyte type and, therefore, the expression values must be extracted by a deconvolution algorithm. 
Within each pair, genes are mixed in 1:2 ratio so that the composite signal should have two peaks: a bigger peak for the cluster to the gene for which a larger proportion of beads are present, and a smaller peak representing the other gene. 

The goal of the contest was to use the data obtained from the DUO method (two genes per analyte type) as input to predict the corresponding measurements obtained from the UNI method (one gene per analyte type) as the ground-truth data.

The data were randomly split into training, testing, and holdout datasets. Training data were made available for all the contestants to develop and validate their solutions offline. The testing data were used to evaluate solutions during the contest and populate the live leaderboard. Holdout data were used evaluate competitors’ final submissions and guard against over-fitting. Prizes were awareded based on performance on the holdout dataset. The datasets are now publicly available [LINK to REPO]. 
...
To evaluate submissions, we developed a scoring function that combines standard measures of accuracy based on the distance (e.g., correlation) between the predicted values and the ground truth (Supporting Info, XXXX), as well as computational speed to evaluate the submissions received and award the prizes to the best competitors. 

The competition run on the platform Topcoder (Wipro, India) for 10 days. A total of $20,000 in cash prizes was offered to competitors as incentive to be divided among the top 9 submissions (with a prize split of 8000, 6000, 4000, 2000, 1000, 800, 600, 400, 100). 
