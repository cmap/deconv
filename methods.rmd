...
The first step towards developing an open innovation competition was to transform the deconvolution problem into a supervised classification task. We generated a novel L1000 dataset of gene-expression values (for 976 genes and xxx compounds and xxx shRNA) based on current CMap's detection method, called DUO, where two genes are measured using the same bead color and expression values must be extracted by a deconvolution algorithm. Then, using the same samples [did we split the samples?], we generated data using another detection method, called UNI, where each gene is measured individually on one single bead color. This method requires twice as many laboratory reagents as DUO and, therefore, it is considerably more expensive, but it does not require deconvolution to extract the expression values. Hence, for the purpose of transforming the problem into a supervised classification task, the data generated under the UNI detection method served as the ground-truth data and the data generated under the DUO detection method served as the data input for the contest, holding out a subset for validation purposes.  All datasets are now publicly available [LINK to REPO]. 
