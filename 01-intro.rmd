...
Here we describe how we addressed these challenges in the context of the Connectivity Map (CMap). The CMap project is a catalog of over 1.3 million human gene-expression profiles of genetic or pharmacologic perturbation. This resource serves multiple purposes in biomedical research applications, such as an efficient way to generate hypothesis for drug discovery and development [@subramanian2017next]. CMap has achieved its massive scale by using a new assay, called L1000. This new assay uses a 500-plex fluorescence-based flow cytometry system to measure a reduced representation of the transcriptome that comprises 1,000 genes highly representative of the state of human cells. By doing so, L1000 achieves a significant cost reduction compared to more traditional methods, such as RNA-sequencing. However, a key technical challenge is to deconvolute the 500 measurements into separate 1,000 gene expression values. The current approach, a k-means algorithm called "dpeak", is slow and can be susceptible to errors. Below we report new methods representing improvements to the current algorithm that we obtained via an open innovation challenge.
