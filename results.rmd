...
Overall, despite the wide diversity of the approaches described above, all top solutions included ways to automatically remove extreme outlier measurements and to incorporate plate level information to improve the predictions. 
...
We tested the accuracy of the solutions by comparing the distribution of the genewise spearman correlation ($\rho$) between the ground-truth gene-expressions (as detected by UNI) and values of the competitors and of the benchmark that were obtained through the deconvolution of DUO data. The Spearman rank correlation was high overall ($\rho$ > `r min(speed_acc$cor)`). However, the empirical distributions of the competitors was right-shifted compared to the k-means benchmark, indicating more accurate predictions in both shRNA and compound experiments (\Fig{accuracy-fig}, A and B). On average, nearly all competitors achieved significant improvements compared to the benchmark (\Fig{accuracy-fig}, C and D), with the best submission raising the average correlation by 3 and 4 percentage points (approximately 5% increase).
