...
```{r spearman-correlation-histograms, fig.cap = "xxx", fig.width = 2.75*2, fig.height = 1.25 * 4}
...
panel <- function(x) {
  x.stat <- median(x)
  plot(ecdf(ifelse(x<0, 0, x)), ann = F, col = COL[1], xlim = x.scale, axes = F)
  abline(v = x.stat, lty = 2, lwd = 2)
  axis(3, at = x.stat, bquote(bar(rho) == .(format(x.stat, digits = 3))))
}
par(mfrow = c(n, 2), mar = c(1,1,3,0), oma = c(4,4,2,1))
for (i in 1:n) { 
  for (plate in c("DPK", "LIT")) {
  h <- handles[i]
  j <- which(ds_cor@cdesc$handle == h & grepl(plate, ds_cor@cdesc$plate))
  x <- ds_cor@mat[, j]
  panel(x)
  j <- which(ds_cor@cdesc$handle == "benchmark" & grepl(plate, ds_cor@cdesc$plate))
  x <- ds_cor@mat[, j]
  lines(ecdf(ifelse(x<0, 0, x)), ann = F, col = COL[2])
  if(plate =="DPK") { 
    if (i == 1) mtext("shRNAs", line = 3)
    i.txt <- ifelse(i=="1", "1st", ifelse(i==2,"2nd",ifelse(i==3,"3rd", paste0(i, "th"))))
     axis(2, at = 0.5, paste(i.txt), las = 1)
  } else { 
    if (i == 1) mtext("Compounds", line = 3)
    txt <- sprintf("%s (%s)", h, method[i])
    legend("topleft", col = COL, c(txt, "benchmark"), bty = "n", pch = 19, text.font = 2)
  }
  if (i == n) axis(1, at = c(0, 0.5, 1))
  }
}
