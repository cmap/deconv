...
![**Schematic illustrating accuracy components of scoring function**. The accuracy component is computed as the product of gene-wise Spearman correlations with ground truth and the area under the curve AUC of extreme modulations.](outcomes/figures/deconvolution_contest Fig2_Final_revised.png)

## Spearman rank correlation
 
```{r spearman-correlation-histograms, fig.cap = "ECDF", fig.width = 2*4, fig.height = 3, eval = F}
...
y.scale <- c(0, 3.5)
panel <- function(x) {
  x.stat <- round(median(x),2)
  plot.stepfun(ecdf(x), ann = F, col = COL[1], xlim = x.scale, axes = F, do.points = F)
  abline(v = x.stat, lty = 2, lwd = 2)
  mtext(side = 3, at = x.stat, text = bquote(bar(rho) == .(format(x.stat, digits = 3))))
}
extract.corr <- function(ds, handle, plate) {
  j <- which(ds@cdesc$handle == handle & grepl(plate, ds@cdesc$plate))
  ifelse(ds@mat[, j]<0,0,ds@mat[, j])
}

for (plate in c("DPK", "LIT")) {
par(mfrow = c(1, n), mar = c(0,0,4,1), oma = c(4,4,3,0))
  for (i in 1:n) { 
    panel(extract.corr(ds_cor, handles[i], plate))
    plot.stepfun(ecdf(extract.corr(ds_cor, "benchmark", plate)), col = COL[2], do.points = F, lty = 2, add = T)
